...
<head>
    ...
    <title>E-Learning: Bước 1 - Khởi tạo Phòng bàn - KiotViet FNB</title>
    ...
    <style>
        body { font-family: 'Inter', sans-serif; background-color: #fff7ed; overflow-x: hidden; } /* Orange background tint */
        ::-webkit-scrollbar { width: 6px; } ::-webkit-scrollbar-thumb { background: #fb923c; border-radius: 3px; }
        .soft-glow-background { position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: -10; background: linear-gradient(135deg, #fff7ed 0%, #ffedd5 50%, #fee2e2 100%); }
        .soft-glow-background .shape { position: absolute; filter: blur(120px); opacity: 0.6; }
        .soft-glow-background .shape1 { height: 300px; width: 300px; background: #fed7aa; border-radius: 50%; top: -50px; left: -50px; animation: moveInCircle 25s ease-in-out infinite; }
        .soft-glow-background .shape2 { height: 350px; width: 350px; background: #fecaca; border-radius: 50%; bottom: -50px; right: -50px; animation: moveVertical 30s ease-in-out infinite; }
        ...
        .nav-link:hover { color: #ea580c; background: rgba(255, 255, 255, 0.6); }
        .nav-link.active { color: #c2410c; border-left: 3px solid #ea580c; background: #ffedd5; font-weight: 600; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05); }
        ...
        .type-card:hover { box-shadow: 0 15px 30px -5px rgba(0, 0, 0, 0.1); transform: translateY(-5px); border-color: #fb923c; }
        ...
        .chat-bubble-consultant { background-color: #fff7ed; color: #c2410c; border-bottom-right-radius: 0.25rem; border: 1px solid #fed7aa; }
...
<body class="text-slate-700 antialiased h-screen flex flex-col overflow-hidden sidebar-closed" id="app-body">
        ...
        <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 h-full">
            <div class="flex items-center justify-between h-full gap-4 lg:gap-8">
                <a href="https://kiotvietlnd.github.io/elearning/" class="flex-shrink-0 flex items-center gap-3 select-none hover:opacity-80 transition-opacity">
                    ...
                    <span class="font-bold text-orange-600 tracking-tight text-base lg:text-lg uppercase">E-Learning</span>
                </a>
                <nav class="hidden lg:flex flex-1 justify-end items-center gap-4">
                    ...
                    <a href="#" class="flex items-center gap-2 px-4 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded-full font-bold text-sm border border-white/20 shadow-lg transform hover:scale-105 transition-all"><i class="fa-solid fa-utensils"></i> Ngành FNB</a>
                </nav>
                <div class="lg:hidden"><button onclick="toggleSidebar()" class="text-slate-600 hover:text-orange-600 text-2xl"><i class="fa-solid fa-bars"></i></button></div>
            </div>
        </div>
    </header>
    ...
    <div class="flex-1 flex overflow-hidden relative">
        ...
        <aside id="sidebar" class="absolute inset-y-0 left-0 z-50 w-80 bg-white/95 backdrop-blur-xl border-r border-white/60 shadow-2xl md:static md:bg-white/60 md:shadow-lg md:rounded-r-2xl flex flex-col transform transition-transform duration-300">
            <div class="p-5 border-b border-slate-100 flex-shrink-0">
                ...
                <h2 class="font-black text-lg leading-tight mb-2 uppercase text-orange-600">Bước 1 - Khởi tạo Phòng bàn</h2>
                <h3 class="font-bold text-slate-500 text-xs uppercase tracking-wider mb-2">TIẾN ĐỘ BÀI HỌC</h3>
                <div class="w-full bg-slate-200 rounded-full h-2"><div id="totalProgress" class="bg-orange-600 hover:bg-orange-700 h-2 rounded-full transition-all duration-500" style="width: 0%"></div></div>
            ...
            <div class="max-w-5xl mx-auto pb-20" id="lessonContainer">
                ...
                <div id="tab-cheatsheet" class="tab-content block animate-fade-in-up">
                    <div class="mb-8 text-center">
                         <span class="bg-orange-100 text-orange-700 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border border-orange-200">Đào tạo Sản phẩm Ngành FNB</span>
                        <h1 class="text-3xl md:text-4xl font-black text-orange-600 mt-4 leading-tight">Bước 1 - Khởi tạo Phòng bàn</h1>
                        <p class="text-slate-500 mt-3 italic text-base md:text-lg max-w-2xl mx-auto">Số hóa không gian quán cafe/nhà hàng: Thiết lập sơ đồ khu vực, vị trí bàn ghế để tối ưu quy trình phục vụ và xếp chỗ.</p>
                    </div>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                            <h3 class="font-bold text-orange-600 mb-3 flex items-center gap-2"><i class="fa-solid fa-bullseye"></i> Mục tiêu</h3>
                            <ul class="space-y-2 text-sm text-slate-600">
                                <li class="flex items-start gap-2"><i class="fa-solid fa-check text-green-500 mt-1"></i> Hiểu vai trò của sơ đồ bàn trong vận hành FNB.</li>
                                <li class="flex items-start gap-2"><i class="fa-solid fa-check text-green-500 mt-1"></i> Biết cách tạo mới bàn thủ công & theo khu vực.</li>
                                <li class="flex items-start gap-2"><i class="fa-solid fa-check text-green-500 mt-1"></i> Sử dụng tính năng Import Excel để tạo hàng loạt.</li>
                            </ul>
                        </div>
                        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                            <h3 class="font-bold text-blue-600 mb-3 flex items-center gap-2"><i class="fa-solid fa-lightbulb"></i> Lợi ích cốt lõi</h3>
                             <ul class="space-y-2 text-sm text-slate-600">
                                <li class="flex items-start gap-2"><i class="fa-solid fa-star text-yellow-400 mt-1"></i> <strong>Trực quan:</strong> Nhân viên dễ dàng nhận biết bàn trống/có khách.</li>
                                <li class="flex items-start gap-2"><i class="fa-solid fa-star text-yellow-400 mt-1"></i> <strong>Tốc độ:</strong> Xếp chỗ và ghi order (gọi món) nhanh chóng.</li>
                                <li class="flex items-start gap-2"><i class="fa-solid fa-star text-yellow-400 mt-1"></i> <strong>Chuyên nghiệp:</strong> Quản lý chặt chẽ khu vực và trạng thái phục vụ.</li>
                            </ul>
                        </div>
                    </div>

                    <div class="bg-orange-50 p-6 rounded-2xl border border-orange-100 mb-8">
                        <h3 class="font-bold text-orange-800 mb-4 text-center uppercase tracking-wider">Quy trình thao tác nhanh</h3>
                        <div class="flex flex-col md:flex-row items-center justify-between gap-4 text-center">
                            <div class="bg-white p-4 rounded-xl shadow-sm w-full md:w-1/4">
                                <div class="w-8 h-8 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center font-bold mx-auto mb-2">1</div>
                                <div class="text-sm font-bold">Vào Menu Phòng/Bàn</div>
                                <div class="text-xs text-slate-500 mt-1">Màn hình Quản lý -> Phòng/Bàn</div>
                            </div>
                            <i class="fa-solid fa-arrow-right text-slate-300 hidden md:block"></i>
                            <div class="bg-white p-4 rounded-xl shadow-sm w-full md:w-1/4">
                                <div class="w-8 h-8 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center font-bold mx-auto mb-2">2</div>
                                <div class="text-sm font-bold">Thêm mới</div>
                                <div class="text-xs text-slate-500 mt-1">Thủ công (+) hoặc Import Excel</div>
                            </div>
                            <i class="fa-solid fa-arrow-right text-slate-300 hidden md:block"></i>
                            <div class="bg-white p-4 rounded-xl shadow-sm w-full md:w-1/4">
                                <div class="w-8 h-8 bg-orange-100 text-orange-600 rounded-full flex items-center justify-center font-bold mx-auto mb-2">3</div>
                                <div class="text-sm font-bold">Điền thông tin</div>
                                <div class="text-xs text-slate-500 mt-1">Tên bàn, Khu vực, Số ghế</div>
                            </div>
                            <i class="fa-solid fa-arrow-right text-slate-300 hidden md:block"></i>
                            <div class="bg-white p-4 rounded-xl shadow-sm w-full md:w-1/4">
                                <div class="w-8 h-8 bg-green-100 text-green-600 rounded-full flex items-center justify-center font-bold mx-auto mb-2">4</div>
                                <div class="text-sm font-bold">Kiểm tra</div>
                                <div class="text-xs text-slate-500 mt-1">Xem sơ đồ trên màn hình Thu ngân</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div id="tab-summary" class="tab-content hidden animate-fade-in-up">
                    <h2 class="text-xl font-bold text-orange-600 mb-6 border-l-4 border-slate-400 pl-3">Bảng so sánh: Lầm tưởng & Sự thật</h2>
                ...
                <div id="tab-game" class="tab-content hidden animate-fade-in-up">
                    <div id="game-start" class="text-center py-20 bg-white/60 backdrop-blur rounded-3xl border border-white/50 shadow-xl">
                        ...
                        <h2 class="text-2xl md:text-3xl font-black text-slate-800 mb-4 tracking-tight">Quiz Game: Kiến Trúc Sư FNB</h2>
                        <p class="text-slate-600 mb-6 max-w-lg mx-auto leading-relaxed text-sm md:text-base">Hệ thống sẽ hiển thị ngẫu nhiên các nhận định về quy trình tạo phòng bàn.<br><strong>Đạt 80% để nhận Chứng chỉ.</strong></p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-3 max-w-3xl mx-auto">
                            ...
                            <button id="btn-mode-pro" onclick="startGame('pro')" class="bg-white border border-purple-200 text-purple-700 px-4 py-3 rounded-lg font-bold shadow-sm text-sm"><div class="text-base mb-1">Pro</div><div class="text-xs font-normal text-slate-500">10 Câu hỏi ngẫu nhiên</div></button>
                    ...
                    <div id="game-playing" class="hidden max-w-2xl mx-auto py-6">
                        ...
                        <div id="game-card" class="bg-white rounded-3xl shadow-xl p-8 min-h-[220px] flex items-center justify-center text-center border border-slate-100 mb-6 relative overflow-hidden transition-all"><div class="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-orange-400 to-red-400"></div><h3 id="question-text" class="text-lg md:text-xl font-extrabold text-slate-800 leading-snug">Loading...</h3></div>
                    ...
                    <div id="game-result" class="hidden flex flex-col items-center">
                        <div id="certificate" class="w-full max-w-4xl aspect-video bg-white shadow-2xl flex overflow-hidden certificate-container mx-auto">
                            <div class="w-1/3 bg-orange-700 flex flex-col items-center justify-center text-white p-6 relative">
                            ...
                            <div class="w-2/3 p-8 md:p-12 flex flex-col justify-center relative bg-white">
                                ...
                                <h1 class="text-3xl md:text-4xl font-black text-orange-800 uppercase mb-1 font-sans tracking-tight">GIẤY CHỨNG NHẬN</h1>
                                ...
                                <div class="space-y-1 mb-6"><p class="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Trao tặng cho:</p><h2 class="text-2xl md:text-3xl font-bold text-orange-600 uppercase leading-tight" id="certificate-name">TÊN HỌC VIÊN</h2></div>
                                <div class="space-y-1 mb-6"><p class="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Học phần:</p><h3 class="text-xl font-bold text-slate-800 uppercase">Bước 1 - Khởi tạo Phòng bàn (FNB)</h3></div>
                        ...
                        <div class="flex flex-wrap justify-center gap-4 mt-8 mb-12">
                            <button onclick="playAgain()" class="bg-white border-2 border-orange-600 text-orange-600 px-6 py-3 rounded-full font-bold shadow-sm hover:bg-orange-50 transition"><i class="fa-solid fa-rotate-right mr-2"></i> Chơi lại</button>
                            ...
                            <button id="btn-download" onclick="downloadCertificate()" class="bg-orange-600 text-white px-8 py-3 rounded-full font-bold shadow-lg hover:bg-orange-700 transition hidden"><i class="fa-solid fa-download mr-2"></i> Tải chứng nhận</button>
                        </div>
                        <div class="max-w-4xl mx-auto bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden w-full">
                            <h3 class="text-xl font-bold text-orange-800 px-6 py-4 bg-slate-50 border-b border-slate-100">Chi tiết kết quả</h3>
    ...
    <script>
        const lessonData = [
            {
                ...
                chat: [
                    { role: "consultant", text: "Chào anh, chào mừng anh đến với KiotViet FNB! Để bắt đầu quản lý nhà hàng/cafe, bước đầu tiên quan trọng nhất là mình phải vẽ lại sơ đồ quán lên phần mềm. Đây là <strong>Bước 1: Khởi tạo Phòng bàn</strong> ạ." },
                    { role: "customer", text: "À, anh hiểu. Nhưng làm cái này để làm gì em? Cứ khách vào ngồi đâu thì ngồi thôi chứ?" },
                    { role: "consultant", text: "Dạ, sơ đồ bàn giúp nhân viên biết ngay bàn nào trống để xếp khách, bàn nào đang chờ món để giục bếp. Nó giúp quán vận hành trơn tru và chuyên nghiệp hơn hẳn đấy ạ." }
                ],
                cards: [
                    {
                        id: 101,
                        isTrue: true,
                        question: "Khởi tạo phòng bàn giúp nhân viên nhận diện bàn trống nhanh chóng?",
                        wrong: "Lầm tưởng: Sơ đồ bàn chỉ để trang trí cho đẹp.",
                        right: "Sự thật: Sơ đồ trực quan giúp nhân viên bao quát toàn bộ trạng thái của quán (trống, có khách, đang thanh toán) chỉ trong 1 giây."
                    },
                    {
                        id: 102,
                        isTrue: false,
                        question: "Phần mềm KiotViet FNB không hỗ trợ quản lý theo Khu vực (Tầng)?",
                        wrong: "Lầm tưởng: Tất cả bàn bị trộn lẫn lộn xộn.",
                        right: "Sự thật: KiotViet cho phép phân chia bàn theo từng Khu vực (Tầng 1, Tầng 2, Sân vườn...) rất khoa học và dễ tìm kiếm."
                    }
                ]
            },
            {
                id: 2,
                title: "Thao tác Tạo bàn",
                icon: "fa-solid fa-chair",
                chat: [
                    { role: "customer", text: "Quán anh mới mở, chỉ có tầm 10 bàn ở tầng trệt thôi. Nhập thế nào em?" },
                    { role: "consultant", text: "Dạ ít thì mình nhập thủ công cho nhanh ạ. Anh vào <strong>Phòng/Bàn -> Thêm mới</strong>. Điền tên 'Bàn 01', chọn Khu vực 'Tầng trệt' và số ghế là xong ạ." },
                    { role: "customer", text: "Thế có ghi chú được 'Bàn cạnh cửa sổ' không em?" },
                    { role: "consultant", text: "Dạ được luôn ạ, có ô Ghi chú để anh điền thoải mái." }
                ],
                cards: [
                    {
                        id: 201,
                        isTrue: true,
                        question: "Có thể ghi chú đặc điểm của bàn (ví dụ: Bàn VIP, Gần cửa sổ)?",
                        wrong: "Lầm tưởng: Chỉ nhập được tên bàn và số ghế.",
                        right: "Sự thật: Bạn có thể thêm ghi chú chi tiết để nhân viên dễ dàng tư vấn chỗ ngồi phù hợp cho khách."
                    },
                    {
                        id: 202,
                        isTrue: false,
                        question: "Bắt buộc phải nhập Số thứ tự cho từng bàn?",
                        wrong: "Lầm tưởng: Không nhập số thứ tự là lỗi.",
                        right: "Sự thật: Số thứ tự là không bắt buộc, nhưng nên nhập để sắp xếp hiển thị trên màn hình thu ngân theo ý muốn."
                    }
                ]
            },
            {
                id: 3,
                title: "Import Excel hàng loạt",
                icon: "fa-solid fa-file-excel",
                chat: [
                    { role: "customer", text: "Nhưng mà quán bạn anh to lắm, 3 tầng, mỗi tầng 10 bàn, tổng 30 bàn lận. Nhập tay từng cái thì đến bao giờ?" },
                    { role: "consultant", text: "Yên tâm ạ, KiotViet có tính năng <strong>Import Excel</strong>. Anh chỉ cần tải file mẫu về, điền 1 loạt 30 bàn vào rồi đẩy lên hệ thống. Tích tắc là xong cả 3 tầng luôn ạ." },
                    { role: "customer", text: "Ồ tiện nhỉ! Thế cột Trạng thái trong Excel điền gì?" },
                    { role: "consultant", text: "Anh điền số 1 là 'Đang hoạt động', điền 0 là 'Ngừng hoạt động' nhé." }
                ],
                cards: [
                    {
                        id: 301,
                        isTrue: true,
                        question: "Có thể tạo hàng loạt phòng bàn bằng cách nhập từ file Excel?",
                        wrong: "Lầm tưởng: Phải nhập tay thủ công từng bàn một dù quán lớn.",
                        right: "Sự thật: KiotViet hỗ trợ import file Excel giúp khởi tạo danh sách bàn lớn cực kỳ nhanh chóng và chính xác."
                    },
                    {
                        id: 302,
                        isTrue: false,
                        question: "Trong file Excel mẫu, cột Trạng thái điền chữ 'Hoạt động'?",
                        wrong: "Lầm tưởng: Điền chữ tiếng Việt vào cột trạng thái.",
                        right: "Sự thật: Cần điền số: 1 (Đang hoạt động) hoặc 0 (Ngừng hoạt động) để hệ thống nhận diện đúng."
                    }
                ]
            },
            {
                id: 4,
                title: "Kiểm tra Sơ đồ",
                icon: "fa-solid fa-desktop",
                chat: [
                    { role: "consultant", text: "Sau khi tạo xong, anh mở màn hình <strong>Thu ngân</strong> lên sẽ thấy ngay thành quả. 30 cái bàn chia 3 tầng hiện ra rất đẹp mắt." },
                    { role: "customer", text: "Công nhận, nhìn trực quan thật. Muốn tìm bàn số 20 thì làm sao?" },
                    { role: "consultant", text: "Anh chỉ cần gõ '20' vào ô tìm kiếm là nó hiện ra ngay lập tức ạ." }
                ],
                cards: [
                    {
                        id: 401,
                        isTrue: true,
                        question: "Trên màn hình Thu ngân, có thể lọc xem bàn theo từng tầng/khu vực?",
                        wrong: "Lầm tưởng: Phải nhìn tất cả các bàn cùng lúc, rất rối mắt.",
                        right: "Sự thật: Bạn dễ dàng chọn tab Khu vực (Tầng 1, Tầng 2...) để chỉ hiển thị các bàn thuộc khu vực đó."
                    }
                ]
            }
        ];

        let gameQuestions = [], currentIndex = 0, score = 0, playerName = "HỌC VIÊN", completedCards = new Set(), isNavRendered = false;
        ...
        function switchTab(tabId) {
            ...
            if(target) target.classList.remove('hidden'); else {
                const dynamicTarget = document.getElementById(tabId); 
                if(dynamicTarget) dynamicTarget.classList.remove('hidden');
            }
            
            document.querySelectorAll('.nav-link').forEach(el => { 
                el.classList.remove('active', 'bg-orange-100', 'text-orange-700', 'border-orange-600'); 
        ...
        function renderNav() {
             ...
             lessonData.forEach(topic => {
                 ...
                 btn.innerHTML = `<i class="${topic.icon} text-orange-500 w-6 text-center"></i><span class="font-bold text-sm truncate">${topic.title}</span>`;
                 ...
                 const chatHTML = topic.chat.map(c => `
                    <div class="mb-4 flex flex-col ${c.role === 'customer' ? 'items-start' : 'items-end'}">
                        <div class="chat-bubble ${c.role === 'customer' ? 'chat-bubble-customer' : 'chat-bubble-consultant'}">
                            <div class="text-[10px] font-bold uppercase mb-1 opacity-70 border-b border-black/5 pb-1 mb-1">${c.role === 'consultant' ? 'Chuyên viên KiotViet' : 'Chủ nhà hàng'}</div>
                            ${c.text}
                        </div>
                    </div>`).join('');
                    ...
                    <div class="h-64 w-full perspective-1000 group cursor-pointer" onclick="flipCard(${card.id}, true)" id="card-container-${card.id}">
                        <div class="card-inner w-full h-full duration-500">
                            <div class="card-face p-6 flex flex-col justify-between items-center text-center border-2 border-slate-100 type-card bg-white">
                                <div class="w-12 h-12 bg-orange-100 rounded-full flex items-center justify-center text-orange-600 font-bold text-lg mb-2">${card.id}</div>
                 ...
                 contentDiv.innerHTML = `
                    <div class="flex items-center gap-3 mb-6"><div class="p-3 bg-orange-100 rounded-xl"><i class="${topic.icon} text-orange-600 text-2xl"></i></div><h2 class="text-2xl font-black text-slate-800">${topic.title}</h2></div>
                    <div class="flex flex-col gap-8 mb-12">
                        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 relative">
                             <div class="border-b border-slate-100 pb-4 mb-4 flex justify-between items-center"><h3 class="font-bold text-slate-500 uppercase text-xs tracking-wider">Hội thoại mẫu</h3><span class="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded">Tình huống giả định</span></div>
                             <div class="py-2 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">${chatHTML}</div>
                        </div>
                        <div>
                             <div class="mb-4 flex justify-between items-end"><h3 class="font-bold text-slate-700 flex items-center gap-2"><i class="fa-solid fa-layer-group text-orange-500"></i> Kiểm tra nhanh kiến thức</h3><span class="text-xs text-slate-400">Chọn Đúng/Sai để lật thẻ</span></div>
             ...
             if (totalQ < 10) {
                 const proBtn = document.getElementById('btn-mode-pro'); if (proBtn) proBtn.style.display = 'none';
                 const btnContainer = document.querySelector('#game-start .grid'); if (btnContainer) btnContainer.classList.replace('md:grid-cols-3', 'md:grid-cols-2');
             }
             isNavRendered = true;
        }
        ...
        function renderSummary() {
            ...
            let allCards = []; lessonData.forEach(t => allCards.push(...t.cards));
            tbody.innerHTML = allCards.map((item, idx) => `
                <tr class="hover:bg-slate-50 border-b border-slate-50 last:border-0"><td class="px-4 py-3 text-slate-400 font-bold text-center align-top">${idx+1}</td>
                    <td class="px-4 py-3 text-slate-600 bg-red-50/30 border-r border-slate-100 align-top"><div class="flex gap-2"><i class="fa-solid fa-xmark text-red-500 mt-1 flex-shrink-0"></i><span class="text-sm">${item.wrong}</span></div></td>
                    <td class="px-4 py-3 text-slate-800 bg-green-50/30 align-top font-medium"><div class="flex gap-2"><i class="fa-solid fa-check text-green-500 mt-1 flex-shrink-0"></i><span class="text-sm">${item.right}</span></div></td></tr>`).join('');
        }

        function flipCard(id, choice) {
            const container = document.getElementById(`card-container-${id}`); const inner = container.querySelector('.card-inner'); const backFace = container.querySelector('.card-back');
            let item; lessonData.forEach(t => { const found = t.cards.find(c => c.id === id); if(found) item = found; });
            
            const isCorrect = (choice === item.isTrue);
            const resultHeader = isCorrect ? '<div class="mb-2 font-bold text-lg text-green-600 flex items-center justify-center gap-2"><i class="fa-solid fa-check-circle"></i> Chính xác!</div>' : '<div class="mb-2 font-bold text-lg text-red-600 flex items-center justify-center gap-2"><i class="fa-solid fa-times-circle"></i> Chưa chính xác!</div>';
            ...
            backFace.innerHTML = `<div class="flex flex-col h-full overflow-y-auto custom-scrollbar">${resultHeader}<div class="mb-4"><span class="px-3 py-1 rounded-full text-[10px] font-bold border ${badgeClass}">${badgeText}</span></div><div class="bg-white p-4 rounded-xl border border-blue-100 shadow-sm text-left flex-1"><p class="text-xs text-blue-600 font-bold uppercase mb-2 flex items-center gap-2"><i class="fa-solid fa-lightbulb"></i> Sự thật là:</p><p class="text-slate-700 text-sm leading-relaxed font-medium">${item.right}</p></div><button onclick="event.stopPropagation(); resetCard(${item.id})" class="mt-4 text-xs text-slate-400 font-bold hover:text-blue-600 uppercase tracking-wider flex items-center justify-center gap-1"><i class="fa-solid fa-rotate-left"></i> Xem lại câu hỏi</button></div>`;
        ...
        function startGame(mode) {
            const nameInput = document.getElementById('player-name').value.trim(); playerName = nameInput || "HỌC VIÊN";
            let allQ = []; lessonData.forEach(t => allQ.push(...t.cards));
        ...
        function loadQuestion() {
            ...
            if (!q) { endGame(); return; } // Safety check
            ...
            btns.forEach(btn => { btn.disabled = false; btn.classList.remove('opacity-50', 'cursor-not-allowed'); });
        }

        function answerGame(choice) {
            const btns = document.querySelectorAll('#game-playing button'); 
            btns.forEach(btn => { btn.disabled = true; btn.classList.add('opacity-50', 'cursor-not-allowed'); });
            
            const q = gameQuestions[currentIndex]; 
            const isCorrect = (choice === q.isTrue);
            
            const row = `<tr class="${isCorrect ? 'bg-green-50' : 'bg-red-50'}"><td class="px-6 py-4 text-slate-700">${q.question}</td><td class="px-6 py-4 text-center font-bold ${choice ? 'text-green-600' : 'text-red-600'}">${choice ? 'ĐÚNG' : 'SAI'}</td><td class="px-6 py-4 text-center">${isCorrect ? '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">Chính xác</span>' : '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">Sai</span>'}</td></tr>`;
            ...
            currentIndex++; 
            if (currentIndex < gameQuestions.length) { setTimeout(loadQuestion, 500); } else { endGame(); }
        }
        ...
        function endGame() {
            ...
            if (percent >= 80) { 
                ...
                failWatermark.innerText = "SAMPLE"; 
